...
public abstract partial class AntagSelectionSystem
{
    ...
    {
        return GetActivePlayerCount(PlayerManager.Sessions);
    }
    ...
    {
        return GetActivePlayers(PlayerManager.Sessions);
    }
    ...
    public IEnumerable<(EntityUid, SessionData, string)> GetAntagIdentifiers(Entity<AntagSelectionComponent?> ent)
    {
        ...
        foreach (var (_, minds) in ent.Comp.AssignedMinds)
        {
            foreach (var (mind, name) in minds)
            {
                ...
                if (!PlayerManager.TryGetPlayerData(mindComp.OriginalOwnerUserId.Value, out var data))
                    continue;
    ...
    public IEnumerable<EntityUid> GetAliveAntags(Entity<AntagSelectionComponent?> ent)
    {
        ...
        {
            if (Mind.IsCharacterDeadIc(mind))
                continue;
    ...
    public abstract void SendBriefing(ICommonSession? session, string briefing, Color? briefingColor, SoundSpecifier? briefingSound);
    ...
    /// Returns a list of all antag players who are constrained by a job whitelist or blacklist from an antag.
    /// </summary>
    /// <param name="except">Antag prototype we're excluding for our returned job whitelist/blacklist.</param>
    /// <returns>A dictionary of antag sessions, and their job blacklists.</returns>
    [PublicAPI]
    public Dictionary<ICommonSession, (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)>
        GetAntagJobs(ProtoId<AntagSpecifierPrototype> except)
    {
        var result = new Dictionary<ICommonSession, (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)>();
        var query = QueryAllRules();
        while (query.MoveNext(out var comp, out _))
    ...
    [PublicAPI]
    public Dictionary<ICommonSession, (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)>
        GetAntagJobs(HashSet<ProtoId<AntagSpecifierPrototype>> except)
    {
        var result = new Dictionary<ICommonSession, (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)>();
        var query = QueryAllRules();
        while (query.MoveNext(out var comp, out _))
    ...
    public (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)
        GetAntagJobs(ICommonSession player, ProtoId<AntagSpecifierPrototype> except)
    {
    ...
    [PublicAPI]
    public (HashSet<ProtoId<JobPrototype>>? Whitelist, HashSet<ProtoId<JobPrototype>>? Blacklist)
        GetAntagJobs(ICommonSession player, HashSet<ProtoId<AntagSpecifierPrototype>> except)
    {
        HashSet<ProtoId<JobPrototype>>? whitelist = null;
        HashSet<ProtoId<JobPrototype>>? blacklist = null;
        var query = QueryAllRules();
    ...
    [PublicAPI]
    public HashSet<ICommonSession> GetPreSelectedAntagSessions(ProtoId<AntagSpecifierPrototype> except)
    {
    ...
    /// <param name="except">A specific definition to be excluded from the check.</param>
    [PublicAPI]
    public HashSet<ICommonSession> GetPreSelectedAntagSessions(HashSet<ProtoId<AntagSpecifierPrototype>> except)
    {
        var result = new HashSet<ICommonSession>();
        var query = QueryAllRules();
        while (query.MoveNext(out var comp, out _))
    ...
    [PublicAPI]
    public abstract IEnumerable<ProtoId<AntagPrototype>> GetValidAntagPreferences(ICommonSession session, List<ProtoId<AntagPrototype>>? filter = null);
    ...
    public bool IsAssignedAntag(Entity<AntagSelectionComponent> gameRule, ICommonSession player)
    {
        ...
        if (Role.PlayerIsAntagonist(player))
            return true;
    ...
    [PublicAPI]
    public bool IsAssignedAntag(ICommonSession player, EntityUid ignored)
    {
        // First check our mindroles.
        if (Role.PlayerIsAntagonist(player))
            return true;
    ...
    /// <returns>True if there is a game rule giving this player antag status</returns>
    [PublicAPI]
    public bool IsAssignedAntag(ICommonSession player, HashSet<EntityUid> ignored)
    {
        // First check our mindroles.
        if (Role.PlayerIsAntagonist(player))
            return true;
    ...
    [PublicAPI]
    public bool IsAssignedExclusiveAntag(ICommonSession player, EntityUid ignored)
    {
        // First check our mindroles.
        if (Mind.GetMind(player.UserId) is { } mind && Role.MindIsExclusiveAntagonist(mind)) // Trauma - use the mind not mob
            return true;
    ...
    /// <returns>True if there is a game rule giving this player antag status that is exclusive with other antags</returns>
    [PublicAPI]
    public bool IsAssignedExclusiveAntag(ICommonSession player, HashSet<EntityUid> ignored)
    {
        // First check our mindroles.
        if (Role.MindIsExclusiveAntagonist(player.AttachedEntity))
            return true;
