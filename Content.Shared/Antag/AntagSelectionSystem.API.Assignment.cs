...
public abstract partial class AntagSelectionSystem
{
    ...
    public bool IsEntityValid([NotNullWhen(true)] EntityUid? uid, AntagSpecifierPrototype def)
    {
        ...
        if (!Whitelist.CheckBoth(uid, def.Blacklist, def.Whitelist))
            return false;
    ...
    [PublicAPI]
    public abstract bool IsAntagBanned(ICommonSession session, AntagSpecifierPrototype definition);
    ...
    public bool TryMakeAntag(Entity<AntagSelectionComponent> gameRule,
        ...
        bool checkPref = true)
    {
        AdminLogger.Add(LogType.AntagSelection,
    ...
    public void SpawnGhostRole(Entity<AntagSelectionComponent> gameRule, AntagSpecifierPrototype proto, bool assert = false)
    {
        ...
        {
            // <Trauma> - replaced with a log so it shows on release too
            if (assert)
                Log.Error($"Tried to spawn a ghost role for {ToPrettyString(gameRule)}:{proto.ID}, but it had no SpawnerPrototype!");
            // </Trauma>
            return;
        }
    ...
    /// </summary>
    protected abstract Entity<AntagSelectionComponent>? ForceGetGameRuleEnt<T>(string id) where T : Component;
