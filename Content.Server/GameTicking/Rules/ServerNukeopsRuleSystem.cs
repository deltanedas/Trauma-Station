...
public sealed partial class ServerNukeopsRuleSystem : NukeopsRuleSystem
{
    ...
    {
        var winText = Loc.GetString($"{rule.Comp.LocalePrefix}{rule.Comp.WinType.ToString().ToLower()}"); // Trauma - use LocalePrefix
        args.AddLine(winText);

        foreach (var cond in rule.Comp.WinConditions)
        {
            var text = Loc.GetString($"{rule.Comp.LocalePrefix}cond-{cond.ToString().ToLower()}"); // Trauma - use LocalePrefix
            args.AddLine(text);
        }
                                     ...
                                     bool announce = true) // Goobstation
    {
        ...
        _antagEvac.SpawnNewAntagIfBelowPercent(ent.Owner, TimeSpan.FromMinutes(3), true); // Trauma
